...
class ModernCollaborationGUI(QMainWindow):
    ...
    def receive_video(self):
        """Receive video from other participants"""
        import socket
            ...
            while self.video_active:
                try:
                    ...
                    if frame is not None:
                        # Store for display (you could add more tiles for multiple users)
                        pass
    ...
    def update_video_frame(self):
        ...
        if self.current_frame is not None:
            ...
            # Scale to fit video tile
            scaled_pixmap = pixmap.scaled(
                self.tile_self.video_label.size(),
                Qt.KeepAspectRatio,
                Qt.SmoothTransformation
            )
            self.tile_self.video_label.setPixmap(scaled_pixmap)
